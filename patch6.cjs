...
function processDirectory(dir) {
    ...
    for (const file of files) {
        ...
        if (fs.statSync(fullPath).isDirectory()) {
            ...
            let newContent = content.replace(regex, (match, prefix, classes, suffix) => {
                const classList = classes.split(/\s+/);
                
                // Only target full-screen centered overlays
                if (classList.includes('fixed') && classList.includes('flex') && classList.includes('items-center') && (classList.includes('inset-0') || classList.includes('inset-x-0'))) {
                    ...
                    if (classList.join(' ') !== newClasses.join(' ')) {
                        modified = true;
                    }
            ...
            newContent = content.replace(regexInner, (match, prefix, classes, suffix) => {
                const classList = classes.split(/\s+/);
                
                // Characteristics of a modal card
                if (classList.includes('bg-white') && (classList.includes('max-h-[90vh]') || classList.includes('max-h-[85vh]')) && classList.includes('flex-col')) {
