...
`;
  ...
  code = code.replace(
    ...
    `<button type="button" onClick={() => { setIsResetting(true); setError(null); setResetSuccess(false); }} className="text-[10px] font-black uppercase tracking-[0.1em] text-brand-green hover:text-emerald-400 transition-colors focus:outline-none">
                      Oubli ?
                    </button>`
  );
                ...
                <form onSubmit={handleReset} className="space-y-8">
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <button
                      ...
                      disabled={loading}
                      className="text-[10px] font-black text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors uppercase tracking-[0.2em] text-center"
                    >
