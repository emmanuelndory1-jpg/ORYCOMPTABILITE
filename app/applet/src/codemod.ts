...
let addition = `
  ...
  const maxClasses = (id: string) => maximizedWidget === id 
    ? "fixed inset-0 sm:inset-4 md:inset-8 z-[100] m-0 !rounded-3xl !max-w-none !col-span-full shadow-2xl overflow-y-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800"
    : "";
  ...
  const MaximizeButton = ({ id }: { id: string }) => (
    <button
      ...
      className="absolute top-4 right-4 p-2.5 rounded-xl bg-slate-100/50 dark:bg-slate-800/50 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-all duration-300 z-[110] opacity-0 group-hover:opacity-100 hover:scale-110 active:scale-95 shadow-sm backdrop-blur-md"
      aria-label="Plein écran"
      title="Plein écran"
    >
      {maximizedWidget === id ? <Minimize2 size={18} /> : <Maximize2 size={18} />}
    </button>
  );
`;
...
// 2. Wrap classNames and add the maximize button!
...
// Update StatCard to support maximize too
