...
export const apiFetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      if (attempt > 0) {
        console.warn(`Retrying fetch for ${input} (attempt ${attempt}/${maxRetries})...`);
      ...
      if (response.status === 403 || response.status === 401) {
        ...
        try {
          ...
          try {
              ...
              if (!isRefreshing) {
                ...
                refreshPromise = (async () => {
                  ...
                  try {
                    ...
                    const refreshRes = await baseFetch('/api/auth/google', {
                      method: 'POST',
            ...
            console.error('Non-JSON response received from API:', input, text.substring(0, 100));
          }
        } catch (e) {
